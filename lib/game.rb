...
class Game
  ...
  def self.create_game
    @players = []
    game_type = Question.question_ask(
      'Select game type: Player vs Player (pp), Computer vs Computer (cc), Player vs Computer (pc) ', 'pp', 'cc', 'pc'
    )
    case game_type
    when 'pp'
  ...
  def turn(player)
    ...
    if player.type == 'Computer'
      answer = @board.board.map.with_index do |cell, index|
        [cell, index + 1]
      end.select { |cell, _index| cell == ' ' }.sample[1]
      @board.board_update(answer, player.symbol)
      puts answer
    elsif player.type == 'Player'
      playable_moves = @board.board.map.with_index do |cell, index|
        [cell, index + 1]
      end.select { |cell, _index| cell == ' ' }.map { |_cell, index| index }
      p playable_moves
      answer = Question.question_ask('Enter your move: ', *playable_moves)

      @board.board_update(answer.to_i, player.symbol) if @board.move_checker(answer.to_i)
    end
    return unless @board.win_checker(player.symbol)
